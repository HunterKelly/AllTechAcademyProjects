...
namespace TheatreCMS3.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(await FindUsername(model.EmailOrUsername), model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
            ...
            //Determine if input is email or username, return proper credentials 
             async Task<string> FindUsername(string input)
            {
                if (IsValidEmail(input))                                                    //If input is Email...
                {
                    ...
                    // checks for null reference returns the input just to show what was invalid
                    if (user == null)
                    {
                        return input;
                    }                   
